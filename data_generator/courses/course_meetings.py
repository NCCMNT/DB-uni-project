...
def fill_course_meetings(cursor: pyodbc.Cursor):
    ...
    limit_of_participants_choice_list = list(range(50, 101, 5))
    ...
    for course_id in range(1, courses_count + 1):
        ...
        for _ in range(no_meetings):
            ...
            values.append([course_id, start_date, end_date, course_instructors_id, limit_of_participants])

        values.sort(key = lambda x: x[2])

        for value in values:
            meeting_id += 1
            course_id, start_date, end_date, course_instructors_id, limit_of_participants = value
